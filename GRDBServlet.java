...
public class GRDBServlet extends HttpServlet
{
    ...
    public   Vector   ListOfMetricsNames  = new Vector() ;
...
/** --------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------- */
    ...
    {
        super.init(config);
	String GrdbDriver = "postgresql.Driver";
        String GrdbUrl = "jdbc:postgresql:grdb";
...
// System.out.println(SqlCommand);	
		...
		} else
		{
                    ...
                    MessagePage message =
                        new MessagePage(ServletAddress, "Invalid Search Text.",
                                        ErrorMessage + "<BR>");
...
//---------------------------------------------------------------------------------
        ...
        ServletOutputStream out = response.getOutputStream();
				...
				DisplayFullRecord = 0 ;
                                ...
                                new MetricDetailPage(ServletAddress, record,DisplayFullRecord);
      ...
      if (user == null) {
        ...
        return;
                                ...
                                MetricModifyPage ModifyPage =
                                new MetricModifyPage(ServletAddress,record);
                        ...
                        else
                        {
                            ...
                            MessagePage message =
                                new MessagePage(ServletAddress,
                                                "Error", ErrorMessage);
                            out.print(message.getHtml());
                        }
                        Result.close();
                   ...
                   //  } catch (SQLException SqlE)
                } catch (Exception SqlE)
                      {
                        ...
                        MessagePage message =
                            new MessagePage(ServletAddress, "Error",
...
// display private input page----------------------------------------------
	...
	PrivateInput privatepage = 
		new PrivateInput(ServletAddress);

                    out.print(privatepage.getHtml());
                }
// enter a new metric------------------------------------------------------

                else if (Command.equals(GRDBProtocol.NEW)) // Enter a new metric
                {
        response.setContentType("text/html");
        ServletOutputStream out = response.getOutputStream();
      // Set the response header to force the browser to
      ...
      if (user == null) {
        ...
        return;
                    ...
                    MetricModifyPage newMetric =
                            new MetricModifyPage(ServletAddress, null);
...
/** --------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------- */
                                       ...
                                       throws ServletException, IOException
    {
        ...
        String Command =
            request.getParameter(GRDBProtocol.COMMAND);

        if (Command == null)
        {
        response.setContentType("text/html");
        ServletOutputStream out = response.getOutputStream();
            GRDBHomePage HomePage =
                    new GRDBHomePage(ServletAddress);
        ...
        ServletOutputStream out = response.getOutputStream();
	     ...
	     String    metriclineelementhtml  = request.getParameter ("metriclineelementhtml ");
                ...
                if (metricname.equals("") ||
                                         metricdimension.equals(""))
                {
                    String ErrorMessage = "The following fields should not be empty :" +
                                    " MetricName, dimension (just as an example)";
                    MessagePage message =
                      new MessagePage(ServletAddress, "Error", ErrorMessage);
                    out.print(message.getHtml());
                    return;
                }
...
// HERE INSERT ENTRY CHECKING
                ...
                try
                {
                  ...
                  {
                    String ErrorMessage = "Dimension must be an integer. " +
                ...
                {
                    String sQuery = "SELECT MAX(metricid) AS " +
                                    "Max_MetricId FROM metrics";
...
SqlCommand = "UPDATE metrics SET metricname='" + metricname + 
			...
			"',metriclineelementhtml='" + metriclineelementhtml + 
                ...
                try
                {
                    ...
                    MessagePage message =
                        new MessagePage(ServletAddress, "Success",
...
// I can here personalize the lists !
              ...
              response.setContentType("text/html");
              ServletOutputStream out = response.getOutputStream();
// it is possible to change this by an input Stream reading 
	      String MetricName = request.getParameter("MetricToLoad");

              try
              {
          if (MetricName != null && !ListOfMetricsNames.contains(MetricName)) {
          ListOfMetricsNames.addElement(request.getParameter("MetricToLoad"));}
                    ConfirmMarking message =
                        new ConfirmMarking(ServletAddress, "Success",
                                     "Record <b>"+MetricName+" </b>marked! Please launch calculator from home page or mark other records.","If the calculator is already launched, please just use the <b>Load/Select</b> option Calculator menu.");
                    out.print(message.getHtml());
                } catch (Exception E)
                  {
...
//out.print(message.getHtml());
			...
			ListOfMetricsNames.removeAllElements();
                    } catch (Exception e)
                      {
	...
	// the results from the Sql Query, one record after the other
		...
		//taking data from result and putting it in the vector of metrics
	     ...
	     record.setmetriclineelementhtml (Result.getString ("metriclineelementhtml"));
                ...
                vRecords.addElement(record); // adding the record to the vector
            }
            ...
            if (vRecords.size() > 0)
            {
                ...
                for (int iCount = 0; iCount < records.length; iCount++)
                    records[iCount] =
                            (MetricRecord)vRecords.elementAt(iCount);
            }
...
// 	System.out.println(GRDBTest.toto+ " le nouveau !!!");
            ...
            SearchResultsPage ResultsPage =
                    new SearchResultsPage(ServletAddress,
                                          records,
                                          "all names");
  ...
  /**
    ...
    */
  protected void login(ServletOutputStream out, String uri)
